...
import config
import constants
from models.smpl import get_smpl_faces, get_model_faces
...
class IUV_Renderer(object):
    def __init__(self, focal_length=5000., orig_size=224, output_size=56, mode='iuv', device=torch.device('cuda'), mesh_type='smpl'):
        ...
        if mode in ['iuv']:
            ...
            with open(os.path.join(config.SMPL_MODEL_DIR, '{}_vert_segmentation.json'.format(body_model)), 'rb') as json_file:
